...
static void *peer_thread(void *arg) {
    ...
    if (exchange(ctx->fd, ctx->is_initiator, out2, sizeof out2, in2, sizeof in2) != 0) return nullptr;
    if (crypto_aead_unlock(peer_pub, in2 + KEY, eph_key, r2_nonce, nullptr, 0, in2, KEY) != 0) return nullptr;
...
/* ---- test 6b: all known X25519 low-order points ------------------------- */

/* The 9 verified low-order points of Curve25519 in Montgomery u-coordinate
 * form (little-endian 32 bytes).  Every scalar multiplication against any of
 * these points yields an all-zero output regardless of the private scalar,
 * because each point lies in the order-8 torsion subgroup or is a non-reduced
 * field representative that reduces to the same projective class.
 *
 * Derivation:
 *   Curve25519 has cofactor 8, giving 8 torsion points with u-coordinates:
 *     order-1/2:  u=0,  u=1,  u=p (≡0), u=p+1 (≡1)
 *     order-4:    u=325606250916557431795983626356110631294008115727848805560023387167927233504
 *                 u=same with sign bit set (high bit of last byte)
 *     order-8:    u=39382357235489614581723060781553021951985786514637840254787674332051736
 *                 u=same with sign bit set
 *   Plus p-1 (≡ -1 mod p), which is also a torsion coordinate.
 *
 * All 9 entries below have been verified empirically: crypto_x25519(priv, u)
 * returns 0x00...00 for three independent clamped private keys.
 */
static void test_x25519_low_order_points(void) {
    printf("\n=== X25519 low-order point rejection ===\n");

    static const uint8_t low_order[9][32] = {
        /* 0: u = 0 */
        ...
        {0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
         0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
        /* 2: u = 325606250916557431795983626356110631294008115727848805560023387167927233504 (order-4) */
        {0xe0,0xeb,0x7a,0x7c,0x3b,0x41,0xb8,0xae,0x16,0x56,0xe3,0xfa,0xf1,0x9f,0xc4,0x6a,
         0xda,0x09,0x8d,0xeb,0x9c,0x32,0xb1,0xfd,0x86,0x62,0x05,0x16,0x5f,0x49,0xb8,0x00},
        /* 3: u = 39382357235489614581723060781553021951985786514637840254787674332051736 (order-8) */
        {0x5f,0x9c,0x95,0xbc,0xa3,0x50,0x8c,0x24,0xb1,0xd0,0xb1,0x55,0x9c,0x83,0xef,0x5b,
         0x04,0x44,0x5c,0xc4,0x58,0x1c,0x8e,0x86,0xd8,0x22,0x4e,0xdd,0xd0,0x9f,0x11,0x57},
        /* 4: u = p = 2^255-19 (non-canonical, ≡ 0 mod p) */
        {0xed,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
         0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x7f},
        /* 5: u = p-1 (≡ -1 mod p) */
        {0xec,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
         0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x7f},
        /* 6: u = p+1 (non-canonical, ≡ 1 mod p — order-2) */
        {0xee,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
         0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x7f},
        /* 7: order-8 point, sign-bit variant (high bit of last byte set) */
        {0x5f,0x9c,0x95,0xbc,0xa3,0x50,0x8c,0x24,0xb1,0xd0,0xb1,0x55,0x9c,0x83,0xef,0x5b,
         0x04,0x44,0x5c,0xc4,0x58,0x1c,0x8e,0x86,0xd8,0x22,0x4e,0xdd,0xd0,0x9f,0x11,0xd7},
        /* 8: order-4 point, sign-bit variant (high bit of last byte set) */
        {0xe0,0xeb,0x7a,0x7c,0x3b,0x41,0xb8,0xae,0x16,0x56,0xe3,0xfa,0xf1,0x9f,0xc4,0x6a,
    ...
    static const char *names[9] = {
        ...
        "u=p (≡0 mod p)",
        "u=p-1 (≡-1 mod p)",
        "u=p+1 (≡1 mod p, order-2)",
        "u=39382...|sign (order-8 variant)",
        "u=325606...|sign (order-4 variant)",
    };

    uint8_t priv[KEY], pub[KEY];
    gen_keypair(priv, pub);

    uint8_t zn1[KEY], zn2[KEY], sas[KEY];
    fill_random(zn1, KEY);
    fill_random(zn2, KEY);

    int dh_zero_count = 0;
    int session_reject_count = 0;

    for (int i = 0; i < 9; i++) {
        uint8_t dh[32];
        crypto_x25519(dh, priv, low_order[i]);
        int dh_is_zero = is_zero32(dh);
        if (dh_is_zero) dh_zero_count++;

        char label_dh[80], label_sess[80];
        snprintf(label_dh,   sizeof label_dh,   "DH output is zero for low-order point %d (%s)", i, names[i]);
        snprintf(label_sess, sizeof label_sess,  "session_init rejects low-order point %d (%s)",  i, names[i]);

        TEST(label_dh, dh_is_zero);

        session_t s;
        int rc = session_init(&s, 1, priv, pub, low_order[i], zn1, zn2, sas);
        int rejected = (rc == -1);
        if (rejected) session_reject_count++;
        TEST(label_sess, rejected);

        crypto_wipe(dh, sizeof dh);
    }

    printf("  low-order DH-zero: %d/9, session rejections: %d/9\n",
           dh_zero_count, session_reject_count);
...
static void *bad_version_peer(void *arg) {
    ...
    /* Send wrong version byte bundled with a dummy commitment + nonce + eph */
    uint8_t eph_priv[KEY], eph_pub_local[KEY];
    gen_keypair(eph_priv, eph_pub_local);
    ...
    /* Send a dummy encrypted key for round 2 so both rounds complete */
    uint8_t dummy_key[KEY], out2[KEY + MAC_SZ], in2[KEY + MAC_SZ];
    fill_random(dummy_key, KEY);
    uint8_t r2_nonce[NONCE_SZ];
    memset(r2_nonce, 0, sizeof r2_nonce);
    crypto_aead_lock(out2, out2 + KEY, eph_key, r2_nonce, nullptr, 0, dummy_key, KEY);
    (void)exchange(ctx->fd, 1, out2, sizeof out2, in2, sizeof in2);
    crypto_wipe(eph_key, sizeof eph_key);
...
static void *bad_commit_peer(void *arg) {
    ...
    /* Bundle version + fake commitment + nonce + eph in round 1 */
    uint8_t eph_priv[KEY], eph_pub_local[KEY];
    gen_keypair(eph_priv, eph_pub_local);
    ...
    /* Round 2: send real pub encrypted — but it won't match the fake commitment */
    uint8_t out2[KEY + MAC_SZ], in2[KEY + MAC_SZ];
    uint8_t r2_nonce[NONCE_SZ];
    memset(r2_nonce, 0, sizeof r2_nonce);
    crypto_aead_lock(out2, out2 + KEY, eph_key, r2_nonce, nullptr, 0, pub, KEY);
    if (exchange(ctx->fd, 1, out2, sizeof out2, in2, sizeof in2) != 0) {
        crypto_wipe(priv, sizeof priv);
        crypto_wipe(eph_key, sizeof eph_key);
        return nullptr;
    }
...
static void *honest_listener(void *arg) {
    ...
    memset(r2_nonce, 0, sizeof r2_nonce);
    crypto_aead_lock(out2, out2 + KEY, eph_key, r2_nonce, nullptr, 0, pub, KEY);
    if (exchange(ctx->fd, 0, out2, sizeof out2, in2, sizeof in2) != 0) {
        crypto_wipe(priv, sizeof priv);
        crypto_wipe(eph_key, sizeof eph_key);
        return nullptr;
    }
...
static void *fp_peer_thread(void *arg) {
    ...
    memcpy(out1 + 1 + KEY, self_nonce, KEY);
    memcpy(out1 + 1 + KEY + KEY, eph_pub_local, KEY);
    if (exchange(ctx->fd, ctx->is_initiator, out1, sizeof out1, in1, sizeof in1) != 0) {
        crypto_wipe(eph_priv, sizeof eph_priv);
        return nullptr;
    }
    uint8_t peer_ver = in1[0];
    ...
    memset(r2_nonce, 0, sizeof r2_nonce);
    crypto_aead_lock(out2, out2 + KEY, eph_key, r2_nonce, nullptr, 0, pub, KEY);
    if (exchange(ctx->fd, ctx->is_initiator, out2, sizeof out2, in2, sizeof in2) != 0) {
        crypto_wipe(eph_key, sizeof eph_key);
        return nullptr;
    }
    if (crypto_aead_unlock(peer_pub, in2 + KEY, eph_key, r2_nonce, nullptr, 0, in2, KEY) != 0) {
...
static void *socks5_server_thread(void *arg) {
    ...
    uint8_t r2_nonce[NONCE_SZ];
    memset(r2_nonce, 0, sizeof r2_nonce);
    crypto_aead_lock(out2, out2 + KEY, eph_key, r2_nonce, NULL, 0, pub, KEY);
    if (exchange(ctx->fd, 0, out2, sizeof out2, in2, sizeof in2) != 0) {
        crypto_wipe(eph_key, sizeof eph_key);
        goto done;
    }
    if (crypto_aead_unlock(peer_pub, in2 + KEY, eph_key, r2_nonce, NULL, 0, in2, KEY) != 0) {
        crypto_wipe(eph_key, sizeof eph_key);
        goto done;
    }
    crypto_wipe(eph_key, sizeof eph_key);
...
static void test_socks5_loopback(void) {
    ...
    uint8_t r2_nonce[NONCE_SZ];
    memset(r2_nonce, 0, sizeof r2_nonce);
    if (hs_ok) crypto_aead_lock(out2, out2 + KEY, eph_key, r2_nonce, NULL, 0, pub, KEY);
    hs_ok = hs_ok && (exchange(client, 1, out2, sizeof out2, in2, sizeof in2) == 0);
    hs_ok = hs_ok && (crypto_aead_unlock(peer_pub, in2 + KEY, eph_key, r2_nonce, NULL, 0, in2, KEY) == 0);
    crypto_wipe(eph_key, sizeof eph_key);

...
static void prepare_percentiles(dudect_ctx_t *ctx) {
    ...
    for (size_t i = 0; i < DUDECT_NUMBER_PERCENTILES; i++) {
        ctx->percentiles[i] =
            percentile(ctx->exec_times, 1 - (pow(0.5, 10 * (double)(i + 1) / DUDECT_NUMBER_PERCENTILES)),
...
#    if DUDECT_TRACE
    ...
    for (size_t i = 0; i < DUDECT_NUMBER_PERCENTILES; i++) { report_test(ctx->ttest_ctxs[i + 1]); }

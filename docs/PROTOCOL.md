...
> **Note:** The `keygen` command creates an *optional* persistent identity key for users who need stable [fingerprints](GLOSSARY.md#fingerprint) across sessions (e.g., paper fingerprint workflow). This does not change the protocol — session keys are still ephemeral and forward secrecy is preserved. The identity key is used only for fingerprint derivation. See [DEPLOYMENT.md](DEPLOYMENT.md) for the use case.

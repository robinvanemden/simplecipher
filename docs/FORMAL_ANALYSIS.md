...
- **TCP backpressure can delay individual frames.** When a non-blocking send does not complete immediately (kernel send buffer full), that frame's wire delivery is extended by TCP drain time. The cover tick schedule advances independently: `next_cover` is always set at tick-fire time, not at drain-completion time, so blocked ticks are absorbed and subsequent intervals remain exponentially distributed. An observer may detect individual frame delivery jitter from TCP conditions, but the inter-tick cadence is not distorted

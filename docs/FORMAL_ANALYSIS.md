...
SimpleCipher's cover traffic uses random intervals in [500, 2500] ms, generated by `cover_delay_ms` using the OS CSPRNG. Cover frames are empty messages (zero-length payload) that are indistinguishable from real messages after encryption (same FRAME_SZ = 512 bytes, same wire padding).

**Formal limitation:** The cover traffic does not provide provable traffic analysis resistance. The random timing distribution is uniform over a fixed interval, which an adversary with sufficient observation time can distinguish from real message timing patterns. Specifically:

- Message bursts (multiple rapid messages) are distinguishable from the uniform cover distribution
- Session start/end times are visible
- Total session duration and frame count are observable

Cover traffic raises the cost of traffic analysis but does not eliminate it.

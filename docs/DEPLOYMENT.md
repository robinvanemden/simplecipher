...
- **Traffic fingerprinting** — all messages use randomized wire padding, but the fixed 512-byte inner frame is a distinguishing feature for a sophisticated observer. Statistical traffic analysis (packet timing, session structure) may also identify the protocol. Tor wraps everything in TLS cells, providing additional protection.

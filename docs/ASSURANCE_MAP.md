...
| Capsicum sandbox (FreeBSD) | `platform.c:capsicum_phase1()` — `cap_enter()` + per-fd rights; `capsicum_phase2()` — narrows socket rights. | `test_p2p.c:test_harden_codepath` — forks child, enters capability mode, verifies `open()` returns ENOTCAPABLE. CI-verified on bare-metal FreeBSD 14.3 (Vultr via SSH) + Cirrus CI. |
| OpenBSD pledge/unveil | `platform.c:sandbox_phase1()` (`"stdio"`), `sandbox_phase2()` (`"stdio"`) | `test_p2p.c:test_harden_codepath` — forks child, calls `pledge("stdio")`, verifies `socket()` triggers SIGABRT. **No OpenBSD CI runner** — test verified manually on OpenBSD. |

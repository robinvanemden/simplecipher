...
| **Post-compromise security** (DH ratchet) | `ratchet.c:ratchet_send/ratchet_receive()` | `test_p2p.c:test_dh_ratchet_pcs`, `test_p2p.c:test_dh_ratchet_deep_pcs` | Fresh DH on each direction switch |
...
| OpenBSD pledge/unveil | `platform.c:sandbox_phase1()` (`"stdio"`), `sandbox_phase2()` (`"stdio"`) | `test_p2p.c:test_harden_codepath` — forks child, calls `pledge("stdio")`, verifies `socket()` triggers SIGABRT. CI-verified on bare-metal OpenBSD 7.7 (Vultr via SSH; when bare-metal hosts are configured — release builds require bare-metal and fail-closed, non-release CI skips gracefully when unconfigured). |
...
| Android cover traffic is connect-mode only | Listener mode has no SOCKS5, so no auto-enable; desktop uses `--cover-traffic` flag | `ANDROID.md:SOCKS5 / Tor support` |

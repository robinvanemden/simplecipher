...
- **[Cover traffic](GLOSSARY.md#cover-traffic) is automatic** when connecting through SOCKS5 (Orbot/Tor). The app uses the queue-on-tick design: real messages are queued and sent on the next cover tick (50-100 ms intervals), replacing the cover payload. All outgoing frames follow the same timing distribution, defeating statistical timing analysis — same mechanism as the desktop `--cover-traffic` flag. Cover traffic is unit-tested (test_p2p.c) and exercised on the desktop SOCKS5 loopback path; the Android emulator CI test exercises the SOCKS5 connect path through the SAS screen but does not reach the post-SAS chat loop where cover traffic starts. Why: the emulator test has a peer (mini_socks5_daemon), but driving both sides through SAS confirmation into the chat loop would require automating the out-of-band SAS exchange between two app instances — the SAS screen deliberately blocks on human input and has no automation bypass, by design.

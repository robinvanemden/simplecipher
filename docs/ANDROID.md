...
- The proxy must be on localhost (`127.0.0.1`, `localhost`, or `::1`). Remote proxies are rejected — a remote proxy adds an untrusted network hop that could stall DNS resolution or TCP connect beyond the per-address timeout, and exposes the target hostname to a third party. This is enforced in both the Java UI and the native JNI layer.
- **[Cover traffic](GLOSSARY.md#cover-traffic) is automatic** when connecting through SOCKS5 (Orbot/Tor). The app uses the queue-on-tick design: real messages are queued and sent on the next cover tick, replacing the cover payload. Inter-frame intervals follow a clamped exponential distribution (mean 500 ms) that mimics natural traffic, significantly mitigating statistical timing analysis — same mechanism as the desktop `--cover-traffic` flag. Note: cover traffic does not defeat protocol fingerprinting; the fixed 512-byte inner frame size and cleartext pad_len are distinguishing features (see [FORMAL_ANALYSIS.md](FORMAL_ANALYSIS.md#75-cover-traffic)). Cover traffic is unit-tested (test_p2p.c) and exercised on the desktop SOCKS5 loopback path; the Android emulator CI test exercises the SOCKS5 connect path through the SAS screen but does not reach the post-SAS chat loop where cover traffic starts. Why: the emulator test has a peer (mini_socks5_daemon), but driving both sides through SAS confirmation into the chat loop would require automating the out-of-band SAS exchange between two app instances — the SAS screen deliberately blocks on human input and has no automation bypass, by design.

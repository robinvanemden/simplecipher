...
| Public key encrypted in handshake round 2 | Round 2 encrypts the public key reveal using an ephemeral X25519 DH established in round 1, preventing passive observers from correlating sessions by public key. An active MITM can still see the keys (they substitute their own ephemeral DH), but the SAS/commitment scheme detects them. | Passive network observers cannot extract the public key. Active MITM is detected by SAS verification. |
...
| Byte-dribble stall | A peer that dribbles bytes or stops reading outbound data can delay I/O completion | POSIX desktop loops use non-blocking I/O (`nb_try_recv`/`nb_try_send`) with incremental accumulation and monotonic deadline checks; Windows uses event-driven state machines; Android uses blocking I/O with deadline-aware helpers (`frame_recv`/`frame_send` with socket timeouts). Maximum stall before the deadline fires is FRAME_TIMEOUT_S (30 seconds). Ctrl+C stays responsive at all times (desktop); Android uses cooperative `running` flag |

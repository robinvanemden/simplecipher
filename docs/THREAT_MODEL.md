...
| Byte-dribble stall (frame_recv/frame_send) | On POSIX, raw mode disables ISIG so Ctrl+C is only handled when stdin is polled. Blocking frame I/O (frame_recv, frame_send) can stall local input and cover scheduling for up to FRAME_TIMEOUT_S (30 seconds) if the peer dribbles bytes or stops reading outbound data | Deadline-aware I/O bounds the stall to 30 seconds (SO_RCVTIMEO + monotonic clock check). Windows uses non-blocking I/O with event-driven state machines, so this does not apply there |
...
### Verified by
- 952 automated tests (926 P2P + 10 SOCKS5 + 16 CLI flag integration)

...
| Version byte not commitment-protected | The version byte is sent in round 1 but is not included in the commitment hash `H(pub\|\|nonce)`. Currently safe (single protocol version), but a future version negotiation scheme must bind the negotiated version into the commitment or IKM to prevent MITM downgrade. | No action needed until v2 is introduced; noted for protocol design. |

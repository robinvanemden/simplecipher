 ...
 *
 * <p>WHY THIS EXISTS: The Android system keyboard (IME) processes every keystroke the user types.
 * Even with IME_FLAG_NO_PERSONALIZED_LEARNING, the IME app still *receives* each character — it
 * just promises not to store it for prediction. But:
 *
 * <p>1. We cannot verify that promise. Third-party keyboards (SwiftKey, Gboard, etc.) are
 * closed-source and may log keystrokes regardless of the flag. 2. The IME runs in a separate
 * process with its own memory space, so even if we wipe our EditText contents, the IME may retain a
 * copy. 3. Some keyboards sync typed text to the cloud for "backup" or "sync across devices"
 * features — sending plaintext messages to a remote server.
 *
 * <p>By building our own keyboard as a View inside the activity, keystrokes never leave our
 * process. We inject characters directly into the EditText via getText().insert(), completely
 * bypassing the InputMethodService pipeline. Combined with showSoftInputOnFocus="false" and
 * InputMethodManager.hideSoftInput, the system keyboard is never shown and never receives input.
 *
 * <p>This is the standard approach used by banking apps, password managers, and other
 * security-sensitive Android applications.
 */
public class SimpleKeyboard extends LinearLayout {
  ...
  public SimpleKeyboard(Context context) {
    this(context, null);
  }
